make_DEG_and_PathFindR <- function(so, cell_type, braak, directory = "SFG_DEG", braak_1= 0){
  ...
  cell_type <- paste0(braak, "_", braak_1, "_", cell_type,"_", directory)
...
net_common <- readr::read_csv("../mouse_tau/tf_tg/rorb_SFG_Mouse_interactions.csv")

tf <- sapply(net_common$N1, function(x){
  tf <- stringr::str_split(x, "_")
  tf <- tf[[1]][1]
})

tg <- sapply(net_common$N1, function(x){
  tf <- stringr::str_split(x, "_")
  tf <- tf[[1]][2]
})

final_net_common <- data.frame(
  tf = toupper(tf),
  tg = toupper(tg)
)


make_DEG_and_PathFindR_TF <- function(so, net, TF, cell_type, braak, directory = "SFG_DEG", braak_1= 0){
  ...
  cell_type <- paste0(braak, "_", braak_1, "_", cell_type,"_",TF,"_", directory)

  f.markers$gene <- rownames(f.markers)

  f.markers <- f.markers[f.markers$gene %in% net$tg,]

  path <- paste0(directory, "/", cell_type,"_", TF, ".csv")
